 ...
 */
export const setUserPreferences = onRequest({ region: "southamerica-west1" },async (req, res) => {
  if (req.method !== "POST") {
  ...
  if (!uid || !Array.isArray(favoritas)) {
    res.status(400).json({ error: "Faltan campos requeridos (uid, favoritas[])" });
    return;
  }
  ...
  try {
    ...
    logger.error("❌ Error al guardar preferencias", { error: error.message || error });
 ...
 */
export const getUserPreferences = onRequest({ region: "southamerica-west1" },async (req, res) => {
  ...
  try {
    ...
    logger.error("❌ Error al obtener preferencias", { error: error.message || error });
